...
class Database(IDataBase):
    ...
    def get_params(self, module_id, time_interval, with_anomalies : bool):

        start_time = int(time.time()) - time_interval * 60 * 60
        params_vect = []
        ...
        response = self.execute_query(sql, [module_id, start_time, with_anomalies])
        cur_params = {}
        for i in range(len(response)):
            cur_params["id"] = response.get_int(i, "id")
            cur_params["module_temp"] = response.get_float(i, "module_temp")
            cur_params["free_bytes"] = response.get_int(i, "free_bytes")
            cur_params["timestamp"] = response.get_int(i, "timestamp")
            params_vect.append(cur_params)
        
        return params_vect
    
    def anomaly_tagging(self, record_ids):
        sql = (
            "UPDATE module_params "
            "SET anomaly = 1 "
            "WHERE id = ?"
        )
        for id in record_ids:
            self.execute_request(sql, [id])

    def get_unique_module_ids(self):
        sql = "SELECT DISTINCT module_id FROM diagnostic_history"
        response = self.execute_query(sql)
        module_ids = []
        for i in range(len(response)):
            module_ids.append(response.get_int(i, "module_id"))
        return module_ids

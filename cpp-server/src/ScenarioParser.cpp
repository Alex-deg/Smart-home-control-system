#include "../include/ScenarioParser.hpp"
...
std::unique_ptr<Node> Parser::parse(const std::string& expression) {
...
std::unique_ptr<Node> ParserImpl::parse() {
...
std::unique_ptr<Node> ParserImpl::parseExpression() {
    auto node = parseTerm();
...
std::unique_ptr<Node> ParserImpl::parseTerm() {
...
std::unique_ptr<Node> ParserImpl::parseFactor() {
    if (pos_ >= tokens_.size())
